...
def arr_grid(arr):
    ...
    for ax in axes:
        ...
        a_min = a[idx].min(); a_max = a[idx].max()
        ...
        ax.matshow(arr[idx], cmap=cm.gray_r, interpolation='nearest', vmin=m_min, vmax = m_max, alpha=0.2)
        idx += 1       
    plt.show()
    plt.close()
...
def de_line(a):
    ...
    a_s = "array...\nshape {} ndim {} size {}\na[0]...\n".format(a.shape, a.ndim, a.size) + a_s
    return a_s

def array2row(arr, with_hdr=True):
    """Calling function"""
    po = np.get_printoptions()
    dim = arr.ndim; shp = arr.ndim
    zz_final = ""
    if dim == 4:
        n = arr.shape[0]
        frmt = '\nArray... {}D  shape: {} size: {}\n3D subarrays: {}\n'
